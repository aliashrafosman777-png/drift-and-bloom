...
export const createProductSchema = z.object({
  ...
  packageCollection: z.string().max(100).optional().default(''),
  size: z.enum(['', 'compact', 'medium', 'large']).optional().default(''),
  difficulty: z.enum(['', 'beginner', 'intermediate', 'advanced']).optional().default(''),
  light: z.enum(['', 'low', 'medium', 'bright']).optional().default(''),
  watering: z.enum(['', 'daily', 'weekly', 'biweekly', 'monthly']).optional().default(''),
  petFriendly: z.boolean().optional().default(false),
  airPurifying: z.boolean().optional().default(false),
